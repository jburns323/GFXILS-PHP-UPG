...
class Network {
	public $Table = 'network_card';
	public $Type = 'Network Card';

	public $Key = 'Network_Card_ID';
	public $SN = 'Network_Card_SN';

	// These get displayed in order
	public $Columns_Unique = array('Network_Card_SN'=>'Network_Card_SN');
	public $Columns = array('Network_Card_Type'=>'Network_Card_Type', 'Location'=>'Location', 'Comments'=>'Network_Card_Comments');

	public $DisplayColumns_Unique = array('Network_Card_SN'=>'Network_Card_SN');
	public $DisplayColumns = array('Network_Card_Type'=>'Network_Card_Type', 'Location'=>'Location', 'Comments'=>'Network_Card_Comments');

	// Column Types
	public $Column_Types = array('Network_Card_SN'=>PDO::PARAM_INT);

	// Required Columns
	public $Columns_Required = array('Network_Card_SN', 'Location');

	public $Columns_Foreign = array('Location'=>'location:Location');

	// These will be dropdown lists of values currently in the table
	public $Columns_List = array('Network_Card_Type', 'Location');

	// These will be freeform textarea inputs instead of one-line text fields
	public $Columns_Text = array('Network_Card_Comments');
	...
	function validate_edit(){
		...
		if (!preg_match('/^\d+$/', $_POST[$this->SN])){
			$this->validate_edit_error = htmlentities($this->SN) . ' must be numeric';
			$this->validate_edit_field = $this->SN;
			return false;
		}
	...
	function validate_duplicate_batch(){
		...
		if (!preg_match('/^\d+$/', $_POST[$this->SN])){
			$this->validate_duplicate_batch_error = $this->SN . ' must be numeric';
